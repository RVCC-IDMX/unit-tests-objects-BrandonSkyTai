...
function getThreeBandValue(bands) {
  const color1 = getColorValue(bands.color1);
  const color2 = getColorValue(bands.color2);
  const multiplier = getMultiplierValue(bands.multiplier);

  let value = (color1 * 10 + color2) * multiplier;
...
function getResistorOhms(bands) {
  ...
  const resistorValue = (color1Value * 10 + color2Value) * multiplierValue;
  const formattedValue = formatNumber(resistorValue);
